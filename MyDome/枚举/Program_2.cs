...
namespace C__dome_1 {
	internal class Program {
		...
		static void Main ( string[] args ) {
			...
			/*
			//Console类
			Console.WriteLine("");		//换行打印
			Console.Write("");			//不换行打印
			Console.ReadLine();			//输入字符串
			Console.ReadKey();			//输入一个字符
			Console.ReadKey(true);      //输入一个字符,加“true”将不在控制台中显示输入的字符
			Console.Clear();            //清除控制台内容
            //Console.Beep(3000 , 3000);	//蜂鸣声
            Console.WriteLine("更改前的背景颜色为"+Console.BackgroundColor);
            Console.BackgroundColor = ConsoleColor.Green;   //更改背景颜色属性为绿色
			Console.WriteLine("更改前的背景颜色为" + Console.BackgroundColor);
			Console.BackgroundColor = ConsoleColor.Black;	//改回原本的黑色
			Console.ForegroundColor = ConsoleColor.Red;		//更改字体颜色为红色
			Console.WriteLine("123456");
			//Environment.Exit(0);							//退出控制台

			*/
			Console.WriteLine("请依次输入：语文，数学，英语，物理，化学，生物的成绩。");
			float[] chengji = new float[6];
			float sum = 0;
			for ( int ij = 0; ij < chengji.Length; ij++ ) {
				chengji[ij] = float.Parse(Console.ReadLine());
				sum += chengji[ij];
			}
            Console.WriteLine("平均成绩为："+sum/chengji.Length);
